...
function start(app) {
  ...
  io.on("connection", function (socket) {
    console.log(
      `Received connection id ${socket.id} connected ${socket.connected}`
    );
  ...
  client.on("message", function (channel, message) {
    ...
    for (var server of servers) {
      ...
      if (server.name == channel) {
        ...
        server.battery = payload.battery;
        updateHealth(server);
      }
    }
  });
  ...
  var latency = setInterval(function () {
    for (var server of servers) {
      if (server.url) {
        ...
        got(server.url, { timeout: 5000, throwHttpErrors: false })
          .then(function (res) {
            //console.log("hello");
            //console.log(res.timings.response - res.timings.start);
...
function updateHealth(server) {
  ...
  score = score + server.memoryLoad < 25.0 ? 1 : 0;
  score = score + server.cpu < 25.0 ? 1 : 0;
  score = score + server.latency < 100 ? 1 : 0;
  score = score + server.statusCode == 200 ? 1 : 0;
