...
{
    let hostpath=path.join(os.homedir(),'mnt');
    ...
    await VBoxManage.execute("modifyvm", `${name} --nic2 bridged --bridgeadapter2 "en0"`)
    await VBoxManage.execute("sharedfolder add", `${name} -name hello -hostpath ${hostpath}`)
    
    //await VBoxManage.execute("hostonlyif create")
...
async function postconfiguration(name)
{
    ...
    ssh("'ls / ; curl -sL https://deb.nodesource.com/setup_12.x | sudo -E bash - ;  sudo apt install -y nodejs ; sudo apt-get update ; sudo apt-get install git ; sudo npm install npm ; git clone https://github.com/CSC-DevOps/App ; cd App ; sudo npm install ; sudo dhclient enp0s8 ; mkdir help ; sudo mount -t vboxsf hello help'");
